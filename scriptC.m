%sliding-window ribbon carving using MEX
%The ribbon carving (function ribboncarvemainC) is executed in C
%input files:'whole_video.avi', 'whole_cost.avi'
...
for flex=1:flexmax
    ...
    while startframe<=Xinfo.NumFrames
        ...
        if endframe<Xinfo.NumFrames
            ...
            clear tempC
            N2=ribboncarvemainC(X,C,flex,eps,vRibbonCount,hRibbonCount);        %do ribbon carving
            X=X(:,:,:,1:N2);
            C=C(:,:,1:N2);
            ...
            N2=ribboncarvemainC(X,C,flex,eps,vRibbonCount,hRibbonCount);
            X=X(:,:,:,1:N2);
            C=C(:,:,1:N2);
            %[X C]=ribboncarvemainv2(X,C,flex);
    ...
    toc
    close(h);
    vertRibbonCount(flex)=vRibbonCount;
    horRibbonCount(flex)=hRibbonCount;    
end
